...
class DiceSpecs extends FunSpec with Matchers {
  ...
  it ("Should return error message when the command is not supported") {
    Initialize()
    (new Dice(StringCommandToWordList("NoSuchCommand 1d6"), ResultCallback)).run()
    actual should be (List("Invalid dice command."))
  }

  it ("Should return error message when the sub command value is invalid.") {
    Initialize()

    (new Dice(StringCommandToWordList("roll 1x1"), ResultCallback)).run()
    actual should be (List("Invalid dice sub command."))
